...
def test_get_certs_that_need_action():
    ...
    # certs to be renewed
    cert_tbr1 = aviacme.cert.Certificate.new(
        "Common", "cert_tbr1", csr, aviacme.cert.ValidationMethod.HTTP01
    )
    cert_tbr1.cert = _generate_certificate(-90800, 4320)
    cert_tbr1.mark_as_installed()
    cert_tbr2 = aviacme.cert.Certificate.new(
        "Common", "cert_tbr2", csr, aviacme.cert.ValidationMethod.HTTP01
    )
    cert_tbr2.cert = _generate_certificate(-9_320_000, 90800)
    cert_tbr2.mark_as_installed()

    # certs to be installed
    cert_tbi1 = aviacme.cert.Certificate.new(
        "Common", "cert_tbi1", csr, aviacme.cert.ValidationMethod.HTTP01
    )
    cert_tbi1.cert = _generate_certificate(-995_700, 1_923_200)
    cert_tbi1.status = aviacme.cert.Status.TO_BE_INSTALLED
    cert_tbi1.save()

    cert_tbi2 = aviacme.cert.Certificate.new(
        "Common", "cert_tbi2", csr, aviacme.cert.ValidationMethod.HTTP01
    )
    cert_tbi2.cert = _generate_certificate(-999_700, 9_123_200)
    cert_tbi2.status = aviacme.cert.Status.TO_BE_INSTALLED
    cert_tbi2.save()

    # cert that neither
    cert_tbnothin = aviacme.cert.Certificate.new(
        "Common", "cert_tbnothin", csr, aviacme.cert.ValidationMethod.HTTP01
    )
    ...
    tbr, tbi = aviacme.cert.get_certs_that_need_action(config)
    tbr_names = tbi_names = []
    for cert in tbr:
...
def test_cert_about_to_expire():
    ...
    not_after_str, _ = aviacme.cert._get_cert_dates(cert)
    assert aviacme.cert._check_if_cert_about_to_expire(not_after_str, 14)
...
def test_cert_not_about_to_expire():
    ...
    not_after_str, _ = aviacme.cert._get_cert_dates(cert)
    assert not aviacme.cert._check_if_cert_about_to_expire(not_after_str, 14)


def test_get_cert_dates():
    cert = _generate_certificate(-10800, 15_552_000)
    actual_nva, actual_nvb = aviacme.cert._get_cert_dates(cert)
...
def test_delete_expired_backups():
    ...
    aviacme.cert.delete_expired_backups()
    assert os.path.isfile("./cert/backup/cert")
    assert not os.path.isfile("./cert/backup/expired_cert")


def test_create_certificate():
    cert = aviacme.cert.Certificate.create("Partition", "Name")
    assert cert.partition == "Partition"
    assert cert.name == "Name"
    assert cert.status == aviacme.cert.Status.NEW
    assert str(cert.path) == "cert/%s_%s.json" % ("Partition", "Name")
    assert cert.validation_method == aviacme.cert.ValidationMethod.HTTP01


def test_new_certificate():
    csr = _generate_csr("common-name", b"DNS:san1,DNS:san2")
    cert = aviacme.cert.Certificate.new(
        "Partition", "test_new_certificate", csr, aviacme.cert.ValidationMethod.DNS01
    )
    assert cert.partition == "Partition"
    assert cert.name == "test_new_certificate"
    assert cert.csr == csr
    assert cert.validation_method == aviacme.cert.ValidationMethod.DNS01


def test_get_non_existing_cert():
    with pytest.raises(aviacme.cert.CertificateNotFoundError):
        aviacme.cert.Certificate.get("Common", "test_get_non_existing_cert")


def test_save_and_get():
    csr = _generate_csr("common-name", b"DNS:san1,DNS:san2")
    cert = aviacme.cert.Certificate.new(
        "Partition", "test_save_and_get", csr, aviacme.cert.ValidationMethod.HTTP01
    )
    cert.save()
    cert2 = aviacme.cert.Certificate.get("Partition", "test_save_and_get")
...
def test_get_without_validation_method():
    ...
    cert = aviacme.cert.Certificate.new(
        ...
        csr,
        aviacme.cert.ValidationMethod.DNS01,
    )
    cert.save()
    with open(cert.path, "r+") as json_bytes:
        ...
        json_bytes.truncate()
    cert2 = aviacme.cert.Certificate.get(
        "Partition", "test_get_without_validation_method"
    )
    assert cert2.validation_method == aviacme.cert.ValidationMethod.HTTP01


def test_save_and_delete():
    csr = _generate_csr("common-name", b"DNS:san1,DNS:san2")
    cert = aviacme.cert.Certificate.new(
        "Common", "test_save_and_delete", csr, aviacme.cert.ValidationMethod.HTTP01
    )
    cert.save()
...
def test_mark_as_installed():
    csr = _generate_csr("common-name", b"DNS:san1,DNS:san2")
    cert = aviacme.cert.Certificate.new(
        "Common", "test_mark_as_installed", csr, aviacme.cert.ValidationMethod.HTTP01
    )
    cert.save()
    assert cert.status == aviacme.cert.Status.NEW
    with open(cert.path, "r") as json_bytes:
    ...
    assert cert.status == aviacme.cert.Status.INSTALLED
    with open(cert.path, "r") as json_bytes:
...
def test_renew():
    csr = _generate_csr("common-name", b"DNS:san1,DNS:san2")
    cert = aviacme.cert.Certificate.new(
        "Common", "test_renew", csr, aviacme.cert.ValidationMethod.HTTP01
    )
    ...
    assert cert.status == aviacme.cert.Status.TO_BE_INSTALLED


def test_up_for_installation():
    csr = _generate_csr("common-name", b"DNS:san1,DNS:san2")
    cert = aviacme.cert.Certificate.new(
        "Common", "test_old_enough", csr, aviacme.cert.ValidationMethod.HTTP01
    )
    cert.cert = _generate_certificate(-1_980_000, 4_320_000)
    cert.status = aviacme.cert.Status.TO_BE_INSTALLED
...
def test_not_up_for_installation_wrong_status():
    ...
    cert.cert = _generate_certificate(-1_980_000, 4_320_000)
    cert.status = aviacme.cert.Status.INSTALLED
...
def test_up_for_renewal():
    csr = _generate_csr("common-name", b"DNS:san1,DNS:san2")
    cert = aviacme.cert.Certificate.new(
        "Common", "test_about_to_expire", csr, aviacme.cert.ValidationMethod.HTTP01
    )
...
def test_not_up_for_renewal():
    csr = _generate_csr("common-name", b"DNS:san1,DNS:san2")
    cert = aviacme.cert.Certificate.new(
        "Common", "test_not_about_to_expire", csr, aviacme.cert.ValidationMethod.HTTP01
    )
...
def test_save_cert_weird_error():
    ...
    cert = aviacme.cert.Certificate.new(
        ...
        csr,
        aviacme.cert.ValidationMethod.HTTP01,
    )
    cert.path = MagicMock()
